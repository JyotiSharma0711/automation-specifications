...
Explicitly out of scope: secured loans, consumer/personal loans, and anchor-led supply chain finance. Any product falling into these categories belongs to a different protocol/spec, not an extension of this one.

This guide explains the BL use case and end-to-end journey from a business and product perspective, before covering the corresponding technical specifications and API flows. Network interactions in this guide reference the ONDC:FIS12:BL domain, version 2.3.0 ([draft-FIS12-2.3.0-BL](https://ondc-official.github.io/ONDC-FIS-Specifications/?branch=draft-FIS12-2.3.0-BL)).
...
Design for partial results — not every lender responds in the same window, so the LSP should render "what's currently known" rather than blocking on the slowest responder. A lender going offline mid-catalogue-refresh shouldn't surface stale or half-updated product data, and products that are paused (not accepting new applications) should be distinguishable from products that are discontinued.
...
The borrower selects exactly one offer. From here, KYC runs for the entity itself, its directors, and any co-applicants — in online or offline mode as required — plus Personal Discussion or Field Verification where the lender's policy requires it.
...
- **`/on_update` — payment details → payment success → quotation updated on the base order**: The lender returns the applicable amount and charges; once paid, confirms success and updates the base-order quotation.

**Network interaction — missed EMI:**

- **`/update`**: The borrower requests missed-EMI payment details.
- **`/on_update` — payment details → payment success on drawdown → base transaction quotation updated**: Same three-step pattern as part-prepayment, scoped to the missed instalment.
...
- **`/on_update` — foreclosure payment details → payment success → base transaction quotation updated**: The lender returns the foreclosure amount and charges; once paid, confirms success and updates the base order.
...
This section wasn't detailed in the source BRD for Business Loans specifically — it's included here for structural consistency with the Personal Loans and LAMF guides, since IGM is a FIS12-wide mechanism rather than a per-product one. Worth confirming with whoever owns the BL BRD whether there are BL-specific grievance scenarios (e.g., disputed drawdown rejections, invoice de-duplication disputes) worth calling out explicitly, the way margin-call disputes are called out for LAMF.
